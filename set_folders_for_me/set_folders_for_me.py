...
\n2. Automatic Mode - A mode where only the name (with complete location) of the text file is given which contains 
a list of names for the folders and the parent folder is named after the name of the text file
\n3. Semi-Automatic Mode - A mode that is used to create folders of the name with a number added to the last of 
the folder name to make them unique from others\n"""
)
...
if mode == 1:
    ...
    if os.path.exists(path):
        ...
        for i in range(folder):
            os.mkdir(
                path + "\\" + name + "\\" + input("Folder " + str(i + 1) + " name: ")
            )
    else:
    ...
    if os.path.exists(path):
        ...
        os.mkdir(path + "\\" + name)
        folder = int(input("How my folders? \n: "))
        cfn = input("Common folder name: ")
        for i in range(1,folder+1):
            os.mkdir(path + "\\" + name + "\\" + cfn +str(i))
    else:
        print("No such path exists. Provide correct path")
    print("Task completed successfully")
else:
